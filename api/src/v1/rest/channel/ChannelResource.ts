...
import {
  AbstractChannel,
  Channel,
} from "../../../../../entities/ts/channels/AbstractChannel";
...
export class ChannelResource {
  ...
  public async getAll(
    ...
    try {
      ...
      const emailChannels = await this.emailRepo.findAllByAndDeepPopulate(
        fieldsToPopulate,
        populateObject
      );
      const opsgenieChannels = await this.opsgenieRepo.findAllByAndDeepPopulate(
        fieldsToPopulate,
        populateObject
      );
      const pagerDutyChannels =
        await this.pagerDutyRepo.findAllByAndDeepPopulate(
          fieldsToPopulate,
          populateObject
        );
      const slackChannels = await this.slackRepo.findAllByAndDeepPopulate(
        fieldsToPopulate,
        populateObject
      );
      const telegramChannels = await this.telegramRepo.findAllByAndDeepPopulate(
        fieldsToPopulate,
        populateObject
      );
      const twilioChannels = await this.twilioRepo.findAllByAndDeepPopulate(
        fieldsToPopulate,
        populateObject
      );

      const channels: Channel[] = [].concat(
        emailChannels,
        opsgenieChannels,
        pagerDutyChannels,
        slackChannels,
  ...
  private async getChannelById(
    ...
    for (const repo of allRepos) {
      if (!channel) {
        channel = await repo.findOneByAndDeepPopulate(id, ["type"], {
          path: "configs",
          select: "sub_chain.name",
        });
      }
    }
  ...
  public async create(
    ...
    try {
      ...
      const ChannelModel = ChannelResource.getModelByChannelTypeValue(
        channelType.value
      );
      const initialisedChannelObject =
        ChannelResource.getInitialisedObjectByChannelTypeValue(
          channelType.value
        );
      ...
      let channel: Channel = MongooseUtil.merge(
        initialisedChannelObject,
        req.body,
        invalidFields
      );
      ...
      if (duplicate) {
        const response = new ResponseError(res, new DuplicateWarning("name"));
        response.send();
        return;
      }
  ...
  public async checkChannelTypeOnCreate(
    req: express.Request,
    res: express.Response
  ): Promise<Generic | ResponseError> {
    if (!("type" in req.body) || typeof req.body.type.id !== "string") {
      const error = new ValidationDataError();
      error.description += " Field type not specified or is not a string.";
      return new ResponseError(res, error);
    }
    const channelType = await this.genericRepo.findOneByGroupAndId(
      "channel_type",
      req.body.type.id
    );
   ...
   */
  private static getConfigTypeIdByChannelTypeValue(
   ...
   */
  private static getModelByChannelTypeValue(
    channelTypeValue: string
  ): Model<any> | null {
    switch (channelTypeValue.toLowerCase()) {
      case "email":
   ...
   */
  private static getInitialisedObjectByChannelTypeValue(
    channelTypeValue: string
  ): Channel | null {
    switch (channelTypeValue.toLowerCase()) {
      case "email":
  ...
  public async update(
    ...
    try {
      ...
      const model: Document<Channel> = MongooseUtil.merge(channel, request);
  ...
  public async createConfigLink(
    ...
    try {
      ...
      if (config_id) {
        const channel = await this.getChannelById(res, req.params.channel_id);
        ...
        // Here we can use email repo since we are using the same collections
        // and IDs are always unique.
        const result = await this.emailRepo.linkConfigToChannel(
  ...
  public async removeConfigLink(
    ...
    try {
      ...
      if (config_id) {
        ...
        // Here we can use email repo since we are using the same collections
        // and IDs are always unique.
        const result = await this.emailRepo.unlinkConfigFromChannel(
          req.params.channel_id,
          config_id
        );
   ...
   */
  private async configValidation(
    req: express.Request,
    res: express.Response
  ): Promise<string> {
   ...
   */
  private isMissingParam(
    req: express.Request,
    res: express.Response,
    channelConfigIdsCheck: boolean = false
  ): ResponseError | null {
    ...
    if (
      channelConfigIdsCheck &&
      (!req.params.channel_id || !req.params.config_id)
    ) {
      const error = new MissingParameterWarning(
        !req.params.channel_id ? "channel_id" : "config_id"
      );
    ...
    if (
      channelConfigIdsCheck &&
      (!mongoose.Types.ObjectId.isValid(req.params.channel_id) ||
