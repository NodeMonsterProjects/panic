...
export class ConfigResource {
  ...
  public async update(
    ...
    try {
      ...
      const config: Document<Config> = (await this.getConfigById(
        res,
        req.params.id
      )) as any;
      if (config instanceof ResponseError) {
  ...
  public async remove(
    ...
    try {
      ...
      const isDeleted = await this.configRepo.deleteOneById(req.params.id);
  ...
  private async save(
    ...
    model.set(
      "config_type",
      new ObjectId(GenericDocument.CONFIG_TYPE_SUB_CHAIN)
    );
    ...
    if (!isValid) {
      ...
      const duplicateError = response.messages.find((message) =>
        message.description.includes("Name duplicated")
      );
      if (duplicateError) {
        response = new ResponseError(
          res,
          new DuplicateWarning(`${duplicateError.name} name`)
        );
      }
  ...
  private async createBkp(config: Document<Config>): Promise<void> {
    ...
    const oldModel: Document<Config> = new ConfigOldModel(config.toObject());
    await oldModel.save();
  }
}
